...
- Shape: a smooth rounded DOME on a low cylindrical base - like half an
  ...
  is only slightly narrower than the dome. The dome is the WIDEST part of
  the lamp; nothing below it is narrower than half the dome's width.
- SIZE, and it is the same on every page: the dome is about TWO THIRDS
  the width of the stool's round seat, so a ring of bare wood shows all
  the way round it. It stands about as tall as it is wide. It is never
  as tall as the stool's legs, and never small enough to look lost on
  the seat. Draw it the same size in every picture.
...
- Beside the pillow, on the LEFT: ONE low wooden stool, knee height, with
  ...
  ITS WOOD IS A WARM DARK REDDISH-BROWN - clearly darker than the pale
  floorboards, the colour of walnut. It is never pale, never blond, never
  orange, never grey. The same tone on every page.
  The stool has a round seat and four straight round legs, with a
  STRAIGHT HORIZONTAL BAR joining the legs partway down, like a simple
  milking stool. That bar is always visible. It is NOT an X, NOT crossed
  and NOT diagonal.
...
- THE CORRIDOR WALLS ARE THE SAME DUSTY BLUE-GREY AS THE BEDROOM AT
  NIGHT, on every corridor page. They are not cream, not white and not
  warm. Only where the lamp's amber pool actually falls does warmth
  appear on them; everywhere else they stay blue-grey.
- The same light wood floor as the bedroom, and a simple skirting board.
"""
