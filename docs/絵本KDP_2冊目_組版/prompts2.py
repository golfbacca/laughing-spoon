...
- ONE small round woven basket, used as the waste basket. It stands on
  the floor DIRECTLY IN FRONT OF the shelf's LEFT-HAND LEG, overlapping
  it, so that leg is HIDDEN BEHIND the basket and cannot be seen past it.
  Not in between the legs, not underneath the shelf, and not out on the
  empty floor away from it. There is only one basket in the whole room.
...
- The shelf is a low TWO-TIER shelf with slim wooden legs. It stands
  against the BACK wall, on the LEFT of the picture, running left-to-right
  across the frame.
  * Its TOP is a CLEAR GLASS PANEL set in a thin wooden frame. You can see
    straight through the glass to the wooden lower shelf underneath, with
    a few soft pale highlights on the glass. The top is NOT a solid wooden
    plank - if you cannot see through it, it is wrong.
  * The LOWER shelf is plain wood, solid and opaque.
...
- The curtain is OPEN, not closed. It is gathered and pushed to ONE SIDE,
  hanging in soft vertical folds, so that beside it the window stands
  OPEN and the morning light pours straight in. The curtain never spreads
  across the picture to cover the window.
- Beside the curtain, where the window is, draw only a soft field of warm
  light - a bright, almost white area with no hard edges.
...
- NO window frame, NO sill, NO glazing bar and NO pane lines anywhere.
  The light simply glows; nothing is outlined. Never draw a short curtain
  with a frame or a sill showing underneath its hem.
"""
...
- THE FABRIC IS OPAQUE. Nothing behind it shows through. Where the
  garment covers a foot, you see only yellow cloth - never a sock, never
  a toe, never a leg and never the floor showing through the fabric. Do
  not draw the outline of anything that is behind it.
"""
...
THE BAG IS ON THE FLOOR IN THIS SCENE, NOT ON THE SHELF. It was taken
down and opened on the previous pages and nobody has put it back yet, so
it stands upright on the floor near the foot of the shelf, on the LEFT.
The top of the shelf is EMPTY.
The bottom third of the picture is plain empty wood floor.
""", [ROOM, PANTS, MODESTY, HEIGHT]),
...
("S13_かべにて", "top third", """
THE SAME MOMENT, ONE BEAT LATER THAN THE STEPPING-IN PICTURE.

COUNT THE FEET. Exactly ONE foot is inside the yellow pants.
- ONE white-socked foot stands on the floor with the soft yellow pants
  BUNCHED COMPACTLY AROUND THAT ONE ANKLE: the ribbed waistband makes a
  soft ring at the ankle and the rest of the fabric gathers in close
  around that same foot, crumpled and low, like a sock pushed down.
- It does NOT stretch out sideways. It does not reach toward the other
  foot and it does not trail away from it. Nothing of it extends past
  that one foot on either side. There is no long tube, no flat sheet, no
  wide spread of fabric on the floor.
- The OTHER foot stands flat on the bare wood floor a little apart, with
  NO fabric on it and NO fabric around it. Clean floor around that foot.
- Nothing is pulled up. Everything stays at ankle level.
...
Mio stands balanced and still, looking down, face calm and settled - the
wobble has passed. No motion marks.
...
THE ROOM MUST MATCH THE OTHER PAGES:
- At the LEFT, the low TWO-TIER wooden shelf on slim legs with an open
  lower shelf, the beige paper bag standing on its top.
- The round woven basket on the OPEN FLOOR beside the shelf.
- At the RIGHT, the cream curtain hanging FULL LENGTH to the floor, with
  no window frame and no sill showing under its hem.
The top third of the picture is plain wall.
""", [ROOM, PANTS, MODESTY, HEIGHT]),
