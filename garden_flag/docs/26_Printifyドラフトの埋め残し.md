...
### ✅ 5商品とも公開できた（2026-09-27）

名入れの文を110字・全大文字0個の版に差し替えたところ、**5商品とも Publish が通った。**
原因は全大文字の語が3つあったことで確定。

Michaelへの返信：

```
That was exactly it. All 5 products published. Thank you, Michael, that saved me a lot of time. Nothing else needed.
```
...
**Printifyの Publish は即公開**なので、5商品は**定価 $39.99 / $74.99 のまま棚に出ている。**

### ⚠ 最優先：30%OFFのセール

```
https://www.etsy.com/your/shops/me/tools/marketing/sales
```

1. `Run a sale` → `Percentage off` → **30**
2. **このガーデンフラッグ5商品だけにチェック。⚠ ゴルフタオル9商品は入れない**
3. 期間 **30日**（広告のテスト期間と合わせる）

→ 表示が **$27.99 / $52.49** になる（`docs/25` の採算どおり）。
⚠ **Etsyのセールは期限で自動的に切れる。** 30日後に判断すること。

### 各商品で3つ（1商品2分）

| | やること |
|---|---|
| ① 写真の並び | `Photo & Video` で **「YOUR FAMILY NAME ON IT」を2番目へドラッグ**。実測でこの1枚だけが最後尾に回る（第7節） |
| ② カテゴリ | `Item Details` の `Banners & Signs` → `Change` → `garden flag` で検索。より具体的なものがあればそちら |
| ③ 属性 | `Item Options` の提案 → `Add all` のあと**値を目視確認**（素材は **Polyester**） |

### R05だけ追加確認

R05は一度**名入れOFFで公開**している。**名入れ欄が付いたか**を確認する。
`Item Options` → `Custom options` に `Personalization / Text box • Required` があればOK。

### テスト品

`Garden & House Banner`（ゴルフタオルの絵）が公開中なら **`Deactivate`**。

### 最後に広告

`docs/22` 第3部。**$3/日 × この5商品だけ。Offsite Ads は触らない。**

⚠ **セールと①〜③が全部終わってから。**
中途半端な状態でクリックを買うと**リスティング品質スコアが下がる**（`docs/04` の教訓）。

### ③ 属性（Attributes）の中身（2026-09-27）

本人から「よくわからない」と質問があったので詳述。

#### 属性とは

**Etsyの検索結果の左側に出る「絞り込み」の材料。**
買い手が `Material: Polyester` で絞ると、**その欄を埋めていない商品は結果から消える。**

**タグとの違い：** タグは打たれた言葉に引っかかるもの。
**属性は埋めていないと候補にすら入らない。**
検索に出ていないことが売れない原因だった（`docs/04`）以上、埋めない理由がない。

#### 場所

`Item Options` タブ → 下へスクロール → **`Attributes`** の見出し。
その上に青い帯で「We've made suggestions using your info and help from AI.」と
**`+ Add all`** ボタン。**AIの推測なので、押したあと必ず値を見る。**
...
| 案 | Primary | Secondary | 実測 |
|---|---|---|---|
| R01 まだ練習中 | **Yellow** | Green | 黄48% / 緑22% |
| R02 19番ホール | **Green** | White | 緑68% / 白30% |
| R05 マリガン | **White** | Pink | 白63% ＋ 桃色のプレート |
| R09 ゴルフ中 | **Red** | Green | 濃い色のうち 赤56% / 緑26% |
| R11 ゴルフ倶楽部 | **Blue** | Gold | 紺91% / 金5% |

⚠ 面積だけで測ると**R11の紺が「黒」と出る**（暗い紺は黒に近いため）。
**色相で分類し直すこと。** 単純な最頻色では誤る。

#### 未確定

`Occasion` `Holiday` などの欄が出るかは**こちらからEtsyが見えないので不明。**
出たら画面を送ってもらって判断する。

#### 優先順位（属性は急がない）

1. **30%OFFのセール**（定価のまま棚に出ているので最優先）
2. 写真の並び直し
3. カテゴリ
4. 属性
5. 広告
