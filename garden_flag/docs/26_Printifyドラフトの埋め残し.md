...
#### 本命：Printifyの「Upload」にこちらの8枚を入れる（要検証）

`Mockups` タブのタイルの中に **「⬆ Upload」** がある。
**ここにこちらの出品画像を入れられるなら、Etsy側の写真作業が丸ごと消える。**

| | 既製を1枚残す場合 | **Uploadが使えた場合** |
|---|---|---|
| Etsyでの写真作業 | 1枚消して8枚アップ × 5商品 | **なし** |
| 再公開したとき | Etsy側の写真が既製品に戻る | **戻らない** |
| `Mockups` の同期チェック | 公開後に外す必要あり | **入れたままでよい** |

出品画像は **2000×1500px（4:3）**。**Etsyの推奨サイズそのもの**なので弾かれる要素はないはず。

**まずR01の1商品だけで試す：**
1. `Mockups` タブ →「Deselect all」
2. 「⬆ Upload」→ `listing/R01_still-working-on-it/` の **1〜8を番号順**に
3. **1番のファイルに `★ Primary` を付ける**（検索結果のサムネイルになる）
4. **ここで止めて画面を送る**

#### 駄目だった場合

**「Deselect all」→ 既製の好きな1枚だけ選ぶ。**
（0枚だとEtsyがリスティングを作れないので1枚は残す）

Etsy側を8枚に入れ替えたら、**`Publishing settings` の `Mockups` のチェックを外す。**
外さないと、**注文ごとに名入れを差し替えて再公開したときに、
